...
public class ConsoleClient : IConsoleClient
{
	...
	private void Graph(IList<string> obj)
	{
		var cache = _cacheManager.ActiveCache;
		var graph = new DotGraph(cache?.Name + "_" + cache?.Id)
		{
			Directed = true
		};

		var internalProjects = _readOnlyProjectProvider.InternalProjectsReadOnly;

		var layerGrouping = internalProjects.Values.GroupBy(x => x.ReferenceLayerReadOnly);

		foreach (var layer in layerGrouping)
		{
			graph.AddSubGraph(layer.Key.ToString(), subGraph =>
			{
				foreach (var readOnlyInternalProject in layer)
				{
					subGraph.AddNode(readOnlyInternalProject.NameReadOnly, node =>
					{
						if (readOnlyInternalProject.DependencyLayerInternalReadOnly is 0)
						{
							node.FillColor = new DotFillColorAttribute(Color.LightCoral);
							node.Style = new DotNodeStyleAttribute(DotNodeStyle.Filled);
						}

						if (readOnlyInternalProject.ReferenceLayerReadOnly is 0)
						{
							node.FillColor = new DotFillColorAttribute(Color.LightBlue);
							node.Style = new DotNodeStyleAttribute(DotNodeStyle.Filled);
						}
					});

					foreach (var readOnlyReference in readOnlyInternalProject.ReferencesReadOnly)
					{
						subGraph.AddEdge(readOnlyReference.Value.UsedByReadOnly.NameReadOnly,
							readOnlyReference.Value.UsingReadOnly.NameReadOnly);
					}
				}

				subGraph.Label = new DotLabelAttribute(layer.Key.ToString());
				subGraph.Color = new DotColorAttribute(Color.Gray);
				subGraph.Style = new DotSubGraphStyleAttribute(DotSubGraphStyle.Solid);
			});
		}

		var file = graph.Compile(true);
		File.WriteAllText(graph.Identifier + ".dot", file);
	}
